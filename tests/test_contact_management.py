...
def test_add_new_contact(app):
    ...
    assert len(old_contact_list) + 1 == len(new_contact_list)  # additional assert for list length in case of exception
...
def test_delete_contact(app):
    ...
    contacts_len_old = len(contacts_list)
    ...
    contacts_list = app.contact_page.get_list()
    # Assert result
    assert contacts_len_old - 1 == len(contacts_list)  # check new length of contacts list
...
def test_delete_first_contact(app):
    ...
    contact_to_delete.id = contacts_list[0].id
    contact_to_delete.first_name = contacts_list[0].first_name
    contact_to_delete.first_name = contacts_list[0].last_name
    # Save length of contact list
    contacts_len_old = len(contacts_list)
